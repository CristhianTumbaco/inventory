$(document).ready(function () {
    ...
    initView();
    bindButtonClick('.btn-reporte', salesReport);
});
const salesFilters = {
    status: '',
    period: 'month',
    ...
    data.forEach(item => {
        ...
        rows += `
            <tr>
                ...
                <td class="fw-bold text-primary">$${(item.total_venta)}</td>
                 <td>${getStatusBadge(item.estado)}</td>
                <td class="text-center user-select-none">
                    <a href="${ruta_web}/Ventas/${item.codigo}" class="btn-admin">
...
function buildStats(data) {
    const statusMap = Object.fromEntries(
        data.statusSales.map(item => [item.estado, item.cantidad])
    );

    const dataformat = {
        totalSales: data.totalSales.cantidad,
        pendingSales: statusMap.Pendiente || 0,
        paidSales: statusMap.Pagada || 0,
        totalMonth: data.totalSales.total || 0
    };
            ...
            <div class="col-md-3">
                <div class="card shadow border mb-1 card-hover h-100">
                    <div class="card-body d-flex align-items-center gap-3 p-2">
                        ...
                        <div>
                            <div class="text-muted fw-bold h5">Total de Ventas</div>
                            <div class="fw-semibold h4">${data.totalSales}</div>
                            <div class="text-muted h6">Todas las ventas</div>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card shadow border mb-1 card-hover h-100">
                    <div class="card-body d-flex align-items-center gap-3 p-2">
                        ...
                        <div>
                            ...
                            <div class="fw-semibold h4">${data.pendingSales}</div>
                ...
                <div class="card shadow border mb-1 card-hover h-100">
                    <div class="card-body d-flex align-items-center gap-3 p-2">
                        ...
                        <div>
                            ...
                            <div class="fw-semibold h4">${data.paidSales}</div>
                            <div class="text-muted h6">Ventas Completadas</div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-md-3">
                <div class="card shadow border mb-1 card-hover h-100">
                    <div class="card-body d-flex align-items-center gap-3 p-2">
                        ...
                        <div>
                            <div class="text-muted fw-bold h5">Total Vendido</div>
...
function dropdownFormat() {
    ...
    document.querySelectorAll('.periodo-item').forEach(item => {
        item.addEventListener('click', function (e) {
            ...
            salesFilters.period = value;
            salesFilters.dateFrom = null;
            salesFilters.dateTo = null;
    ...
    document.getElementById('btnAplicarRango')?.addEventListener('click', function () {
        ...
        salesFilters.dateFrom = desde;
        salesFilters.dateTo = hasta;
    ...
    sendAjax({
        ...
        onSuccess: (data) => {
            if (data.success) {
                ...
                exportToExcel(datos, "ReporteVentas");
            } else {
                Notify(2, data.message);
            }
        }
